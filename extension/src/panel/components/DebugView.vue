...
import { Server, Radio, Database, Activity, RefreshCw, ArrowLeft } from "lucide-vue-next";
...
async function refresh() {
  try {
    ...
    // store.error handles user-facing message.
  }
}
...
<template>
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-6">
      <div class="grid grid-cols-1 gap-3">
        <div class="p-3 bg-ui-surface border border-ui-border rounded-sm space-y-2">
          ...
          <p class="text-[11px] font-mono text-ui-text-muted truncate opacity-80">{{ health.bridgeUrl }}</p>
        </div>
        ...
        <div class="p-3 bg-ui-surface border border-ui-border rounded-sm space-y-2">
          ...
          <p class="text-[11px] font-mono text-ui-text-muted truncate opacity-80 uppercase">{{ health.llmModel }}</p>
        </div>
      </div>

      <section class="space-y-3">
        <h3 class="text-[10px] font-bold text-ui-text-muted/70 uppercase tracking-[0.2em] px-1">Active Protocols</h3>
        <div class="space-y-2">
          <div class="flex items-center justify-between p-3 border border-ui-border rounded-sm hover:bg-ui-surface transition-colors">
            <div class="flex items-center gap-3">
              <Database :size="16" class="text-ui-text-muted opacity-60" />
          ...
          <div class="flex items-center justify-between p-3 border border-ui-border rounded-sm hover:bg-ui-surface transition-colors">
            <div class="flex items-center gap-3">
              <Activity :size="16" class="text-ui-text-muted opacity-60" />
    ...
    <footer class="p-4 border-t border-ui-border bg-ui-surface/20">
      <button
        class="w-full bg-ui-bg border border-ui-border py-2.5 rounded-sm text-[12px] font-bold flex items-center justify-center gap-2 hover:bg-ui-surface transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ui-accent"
        :disabled="loading"
        @click="refresh"
      >

...
<template>
  <div 
    v-if="isOpen"
    class="fixed inset-0 z-50 bg-ui-bg flex flex-col transition-transform duration-200 ease-in-out"
    :class="isOpen ? 'translate-x-0' : '-translate-x-full'"
  >
    <!-- Header: Refined 16px icons -->
    <header class="h-12 flex items-center px-2 border-b border-ui-border bg-ui-bg shrink-0">
      <button 
        class="p-2 hover:bg-ui-surface rounded-md text-ui-text transition-colors"
        @click="$emit('close')"
      >
        <ArrowLeft :size="16" />
      </button>
      ...
      <button 
        class="ml-auto p-2 hover:bg-ui-surface rounded-md text-ui-text transition-colors"
        @click="$emit('new')"
      >
        <Plus :size="16" />
      </button>
    ...
    <div class="px-3 py-2 border-b border-ui-border">
      <div class="relative group">
        ...
        <input 
          ...
          type="text" 
          placeholder="搜索会话记录..."
          class="w-full pl-8 pr-3 py-1.5 bg-ui-surface border-none rounded-md text-[12px] text-ui-text focus:ring-1 focus:ring-ui-accent/20 transition-all placeholder:text-ui-text-muted/50"
        />
      </div>
    </div>

    <!-- Refined Session List -->
    <nav class="flex-1 overflow-y-auto px-1.5 py-2 space-y-0.5 custom-scrollbar">
      <div 
      ...
      >
        <button 
          class="w-full flex items-start gap-2.5 px-3 py-2.5 rounded-md transition-all text-left relative overflow-hidden"
          :class="[
            activeId === session.id 
        ...
        <!-- Tool Actions: 14px icons -->
        <div 
          class="absolute right-2 top-1/2 -translate-y-1/2 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity"
        >
          <button 
            class="p-1.5 text-ui-text-muted hover:text-ui-accent transition-colors"
            @click.stop="$emit('refresh', session.id)"
          >
            <RefreshCcw :size="11" />
          </button>
          <button 
            class="p-1.5 text-ui-text-muted hover:text-red-500 transition-colors"

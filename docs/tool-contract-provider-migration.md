...
1. 已完成：extension 侧已落 `ToolProviderRegistry + CapabilityPolicyRegistry + PluginRuntime`，`executeStep` 支持 capability/provider 路由与策略覆盖；`ToolProviderRegistry` 已支持同 capability 多 provider（`priority + canHandle`）路由。

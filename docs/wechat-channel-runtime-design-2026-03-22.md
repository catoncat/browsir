...
- 插件 chat 是 full-fidelity surface；微信只是 terminal text projection surface。
- 微信不接工具调用、不接中间过程、不接 progress/status；只接终态文本结果。
- 终态文本结果既可以是 LLM 最终回复，也可以是 `denied / safe_failure / cancelled / expired / error` 这类系统终态文本。
- 渠道来源提示只做轻量会话级标识，例如标题 badge 或 icon，不改聊天流本身。
...
- 插件 chat 保持完整显示，不为微信入口另造一套特殊聊天 UI
- 微信只拿终态文本投影，不拿工具调用和中间过程
...
```text
SidePanel UI
  -> full-fidelity chat surface
  -> settings/status/debug entry
...
- chat 继续显示完整会话内容
- 轻量标注当前会话来自微信
- 设置页提供登录按钮、通道开关、状态和最近错误
- richer debug 作为二级入口，不作为主体验
...
这样 `ChannelTurn -> assistant result commit -> projection -> 发回微信` 这条链路才是稳定的，而插件 chat 仍然可以完整显示这段会话。
...
V0 可以先不做复杂权限系统，但设计上仍然不能默认“微信消息 = 插件本地输入”，哪怕它们最后落在同一个会话里。
...
当 remote request 触发本地确认时：

- 插件 chat 里进入同一会话的确认状态
- 本地继续走现有 intervention state machine
- approval / cancel / timeout 最后再回投影成微信可见的终态文本
...
- 设置入口先放进 `SettingsView.vue`
- chat 面只补轻量来源标识，不改完整消息流
...
微信只发“用户可见终态文本”，不发内部调试上下文，也不发中间过程。
...
不要把工具调用、中间状态或 progress/status 发到微信。
...
2. 微信入站文本创建或复用专用 session，插件 chat 仍完整显示会话内容
3. session 正在运行时第二条消息走 `followUp`
4. assistant result commit 后只发送一次对应 `channelTurnId` 的终态文本结果
5. 工具调用和中间过程不会投影到微信
6. 重复 `message_id` 不会二次触发
7. offscreen 重启后可恢复轮询状态
8. token 失效后状态切回登录态
...
2. 插件 chat 保持完整显示，与现有 chat 体验一致
3. 多轮上下文连续，`remote conversation -> session` 映射稳定
4. channel transport 与 agent decision-making 明确分层
5. 发回微信的是经过 projection 的终态文本结果，不是内部 transcript、中间过程或工具调用
6. 未来新增第二个 channel 时，主要工作看起来像新增 adapter，而不是复制一套 bot system

...
4. **保留并收紧 `use-runtime-messages.ts` 边界**
   - 当前工作树中 `use-runtime-messages.ts` 已经接入 `ChatView.vue`，runtime message bus / polling / bridge status wiring 已有独立归宿
   - 下一步不是重复抽这一层，而是避免消息接线逻辑重新回流进视图主体，并在需要时继续把该 composable 细拆为 event dispatch / polling sync / bridge status 三层
...
- `extension/src/panel/composables/use-runtime-messages.ts`（已接线，继续保持边界稳定）
...
- [ ] `use-runtime-messages.ts` 继续保持已接线的独立边界，不回流到 `ChatView.vue`
- [ ] `ChatView.vue` 首轮不再同时内联 toolbar / conversation actions 与大块 view-local watch glue 等剩余热点

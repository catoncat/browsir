...
5. **从 `ChatView.vue` 抽 panel UI plugin runtime / render hook 管线**
   - 建议独立为 `use-panel-ui-runtime.ts`（或等价边界）
   - 收口 hydrate、extension lifecycle、render hook payload normalize/patch、notice 体系

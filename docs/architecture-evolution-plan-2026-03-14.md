...
3. **继续从 `ChatView.vue` 抽运行态控制器**
   - 当前工作树已存在 `use-llm-streaming.ts`，首轮把流式草稿状态从主文件中移出
   - 下一步推荐目标：`use-tool-pending-state.ts`
   - 剩余范围：tool pending / run-view phase / step stream sync / tool card derived state
...
- [ ] `ChatView.vue` 首轮不再同时内联 tool pending / run-view 主状态机与 runtime message bus/polling 两大剩余热点

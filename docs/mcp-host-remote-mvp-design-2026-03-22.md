...
- extension 只保存 server metadata 与 `authRef/envRef`
- Bridge 持有 bearer token / OAuth client secret / 自定义 header
...
- 扩展侧 `McpServerConfig` 只保存 `authRef/envRef`
- Bridge 侧通过 secret store 解引用真实凭证
...
```ts
interface McpSecretRef {
  id: string;
  kind: "env" | "bearer" | "oauth-client-credentials";
}
```
